...
export function EditTicketPage() {
  ...
  const loadTicket = async () => {
    try {
      ...
      setGameDate(formatDateForInput(ticket.gameDate));
      setAmount(ticket.amount !== null ? String(ticket.amount) : '');
      setPlace(ticket.place || '');
      setStatus(ticket.status);
      setNotes(ticket.notes || '');
    } catch {
      setApiError('No se pudo cargar la boleta para editar.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="page-container animate-fade-in-up">
      <div className="ticket-form-container">
        <div className="page-header">
          <h1 className="page-title">✏️ Editar Boleta</h1>
          <p className="page-subtitle">Modifica la información registrada de tu boleta</p>
        </div>
        ...
        <div className="auth-card" style={{ padding: 'var(--space-xl)' }}>
          ...
          <form className="ticket-form" onSubmit={handleSubmit} noValidate>
            ...
            <div className="ticket-form-actions">
              <Button type="button" variant="ghost" onClick={() => navigate(`/tickets/${id}`)}>
                Cancelar
              </Button>
